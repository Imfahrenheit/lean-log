...
export async function GET(request: Request) {
  ...
  // Exchange code for session (only if code is present)
  if (code) {
    ...
    try {
      ...
      if (error) {
        ...
        if (error.message?.includes("code verifier") || error.message?.includes("auth code")) {
          return NextResponse.redirect(
            `${origin}/signin?error=${encodeURIComponent("Authentication error. Please try signing in again with the magic link.")}`
          );
        }
        return NextResponse.redirect(
      ...
      return NextResponse.redirect(
        `${origin}/signin?error=${encodeURIComponent("Authentication error. Please try again.")}`
      );
    }
  }
